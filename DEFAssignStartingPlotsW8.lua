...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		AddForestToResource = AssignStartingPlots.AddForestToResource,
		...
		iNumMaxAllowedForRegions = 10,	-- Maximum luxury types allowed to be assigned to regional distribution. CANNOT be reduced below 8!
		...
		centerBias = 46, -- % of radius from region center to examine first
		...
		wheat_ID, cow_ID, deer_ID, banana_ID, fish_ID, sheep_ID, stone_ID, hardwood_ID, maize_ID,
		...
		bison_ID,
		-- Mod luxuries
		coffee_ID, tea_ID, tobacco_ID, amber_ID, jade_ID, olives_ID, perfume_ID, coral_ID, lapis_ID, obsidian_ID,
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		elseif resourceType == "RESOURCE_OBSIDIAN" then	-- MOD.Barathor: New
...
function AssignStartingPlots:DetermineRegionTypes()
	...
	for this_region, terrainCounts in ipairs(self.regionTerrainCounts) do
		...
		print("--"); print("AreaPlots: ", areaPlots);
		print("--"); print("MarshPlots: ", marshCount);
		print("--"); print("FloodPlots: ", floodplainCount);
		...
		-- Hills check.
		elseif (hillsCount >= areaPlots * 0.35) then
		...
		-- Wetlands check.
		elseif ((marshCount + floodplainCount) >= areaPlots * 0.12) or (floodplainCount >= 9) or (marshCount >= 7) then
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	local ripple_values = {98, 96, 94, 92, 88, 82, 74, 66, 56, 46, 36, 28, 20, 12};
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis)
	...
	elseif featureType == FeatureTypes.FEATURE_FOREST then -- Place Deer
		...
		return true, false
	elseif plotType == PlotTypes.PLOT_HILLS and featureType == FeatureTypes.NO_FEATURE and plot:IsFreshWater() then
...
function AssignStartingPlots:AddStrategicBalanceResources(region_number)
	...
	local even = self.firstRingYIsEven;
	local nextX, nextY, plot_adjustments;
	local iron_list, horse_list, oil_list, alum_list, coal_list = {}, {}, {}, {}, {};
	local iron_fallback, horse_fallback, oil_fallback, alum_fallback, coal_fallback = {}, {}, {}, {}, {};
	local radius = 3;
	...
	local shuf_list;
	local placed_iron, placed_horse, placed_oil, placed_alum, placed_coal = false, false, false, false, false;
	...
	if self.resource_setting == 7 or self.resource_setting == 8 or self.resource_setting == 4 then
	...
	if self.resource_setting == 6 or self.resource_setting == 8 or self.resource_setting == 4 then
	...
	if self.resource_setting == 7 or self.resource_setting == 8 then
		if placed_alum == false and table.maxn(alum_fallback) > 0 then
			shuf_list = GetShuffledCopyOfTable(horse_fallback)
			iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.aluminum_ID, alum_amt, 1, 1, 1, 0, 0, shuf_list);
		end
	end
	if self.resource_setting == 6 or self.resource_setting == 8 then
...
function AssignStartingPlots:NormalizeStartLocation(region_number)
	...
	print("Resource Setting: ", self.resource_setting);
	
	-- Add mandatory Iron, Horse, Oil to every start if Strategic Balance option is enabled.
	if self.resource_setting == 5 or self.resource_setting == 6 or self.resource_setting == 7 or self.resource_setting == 8 or self.resource_setting == 4 then
		print("\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/");
		print("Adding Bonues To Region", region_number);
		print("\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/");
			...
			for plot_to_handle = 1, ripple_radius do
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 then -- No resource here, safe to proceed.
						...
						if plotType == PlotTypes.PLOT_LAND then
							if terrainType == TerrainTypes.TERRAIN_PLAINS or terrainType == TerrainTypes.TERRAIN_TUNDRA then
								if featureType == FeatureTypes.NO_FEATURE then
	...
	if iNumFoodBonusNeeded > 0 then
		...
		while iNumFoodBonusNeeded > 0 do
			if ((innerPlaced < 3 and innerCanHaveBonus > 0) or (self.resource_setting == 4 and innerPlaced < 3 and innerCanHaveBonus > 0))
...
function isMiddle(x)
	...
	local midRangeLow = iW * 0.5 - 2;
	local midRangeHigh = iW * 0.5 + 1;
	
	if x >= midRangeLow and x <= midRangeHigh then
		return true;
	end
	return false;
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
	...
	--
	local iW, iH = Map.GetGridSize();
	local temp_hills_list, temp_coast_list, temp_front_coast_list, temp_grass_flat_no_feature = {}, {}, {}, {};
					...
					if featureType ~= self.feature_atoll then
						...
						elseif terrainType == TerrainTypes.TERRAIN_COAST then
							...
							if plot:IsAdjacentToLand() then
								if isMiddle(x) == false then 
									table.insert(temp_coast_next_to_land_list, i);
								else
									table.insert(temp_front_coast_list, i);
								end
							end
						else
							self.barren_plots = self.barren_plots + 1;
	...
	self.front_coast_list = GetShuffledCopyOfTable(temp_front_coast_list)
...
function AssignStartingPlots:PlaceResourceImpact(x, y, impact_table_number, radius)
	...
	elseif impact_table_number == 7 then
		self.marbleData[impactPlotIndex] = 2;
	end
	if radius == 0 then
		return
	end
			...
			for direction_index = 1, 6 do
				for plot_to_handle = 1, ripple_radius do
					...
					elseif wrapY == false and (nextY < 0 or nextY >= iH) then -- Y is out of bounds.
						...
						elseif impact_table_number == 7 then
							self.marbleData[ringPlotIndex] = 2;
						end
					end
					currentX, currentY = nextX, nextY;
				end
	...
	if iNumAvailableTypes == 0 then
		...
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			...
			if res_ID ~= nil and self.luxury_assignment_count[res_ID] ~= nil and self.luxury_assignment_count[res_ID] < 3 then -- This type still eligible.
				local test = TestMembership(self.resourceIDs_assigned_to_regions, res_ID)
				if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Won't exceed allowed types.
					if res_ID ~= self.whale_ID and res_ID ~= self.pearls_ID and res_ID ~= self.crab_ID and res_ID ~= self.coral_ID then
					table.insert(resource_IDs, res_ID);
					local adjusted_weight = resource_options[2] / (1 + self.luxury_assignment_count[res_ID])
					table.insert(resource_weights, adjusted_weight);
					iNumAvailableTypes = iNumAvailableTypes + 1;
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.ivory_ID then
		primary, secondary, tertiary = 11, 12, 13, 4;
	elseif resource_ID == self.fur_ID then
		primary, secondary, tertiary = 14, 12, 11, 4;
	elseif resource_ID == self.dye_ID then
		primary, secondary, tertiary, quaternary = 9, 8, 2, 4;
	elseif resource_ID == self.spices_ID then
		primary, secondary, tertiary, quaternary = 8, 15, 2, 4;
	elseif resource_ID == self.silk_ID then
		primary, secondary, tertiary = 15, 8, 4, 5;
	elseif resource_ID == self.sugar_ID then
		primary, secondary, tertiary, quaternary = 2, 13, 9, 3;
	elseif resource_ID == self.cotton_ID then
		primary, secondary, tertiary, quaternary = 3, 13, 12, 4;
	elseif resource_ID == self.wine_ID then
		primary, secondary, tertiary, quaternary = 11, 12, 13, 4;
	elseif resource_ID == self.incense_ID then
		primary, secondary, tertiary, quaternary = 11, 10, 3, 4;
	...
	elseif resource_ID == self.salt_ID then
		primary, secondary, tertiary, quaternary = 11, 9, 4, 5;
...
function AssignStartingPlots:PlaceLuxuries()
	...
	local rtoplace = 1;
	local RndTable = {};
	local RndTable = table.fill(1, self.iNumCivs);
	local acttoplace = 0;
	local rmore = false;
	local lefttoplace = 0;
	...
	for loop, reg_data in ipairs(self.regions_sorted_by_type) do
		...
		if self.regionData[region_number] ~= nil and self.regionData[region_number][8] ~= nil and self.regionData[region_number][8] < 2.5 then -- Low fertility per region rectangle plot, add a lux. 2.5
			print("-"); print("Region#", region_number, "has low rectangle fertility, extra regional lux deferred off the capital.");
		...
		if landPlots > 0 and self.regionData[region_number] ~= nil and self.regionData[region_number][6] ~= nil and self.regionData[region_number][6] / landPlots < 4 then -- Low fertility per land plot. 4
			print("-"); print("Region#", region_number, "has low per-plot fertility, extra regional lux deferred off the capital.");
			if this_region_luxury ~= nil then
				local c = self.luxury_low_fert_compensation[this_region_luxury];
				if c == nil then
					c = 0;
	...
	if self.resource_setting ~= 1 then
		for region_number = 1, self.iNumCivs do
			...
			print("--"); print("Start Location: ", x, y, "Plot Location: ", start_plot)
			...
			local allowed_luxuries = self:GetListOfAllowableLuxuriesAtCitySite(x, y, 2, coastal_start)
			...
			while lplaced == false do
				...
				print("Num Ramdoms: ", self.iNumTypesRandom);
				print("Types Allowed: ", iNumTypesAllowed);
				print("Selected Lux: ", use_this_ID);
				...
				if use_this_ID ~= nil and use_this_ID ~= 0 then -- Place this luxury type at this start.
					...
					luxury_plot_lists = self:GenerateLuxuryPlotListsAtCitySite(x, y, 2, false)
					shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[primary])
					local iNumLeftToPlace = self:PlaceSpecificNumberOfResources(use_this_ID, 1, acttoplace, 1, 2, 2, 0, shuf_list);
					acttoplace = iNumLeftToPlace; 
					if iNumLeftToPlace > 0 and secondary > 0 then
						shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[secondary])
						iNumLeftToPlace = self:PlaceSpecificNumberOfResources(use_this_ID, 1, acttoplace, 1, -1, 0, 0, shuf_list);
						acttoplace = iNumLeftToPlace;
					end
					if iNumLeftToPlace > 0 and tertiary > 0 then
						shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[tertiary])
					...
					if iNumLeftToPlace == 0 and lefttoplace == 0 then
						...
						print("Left To Place: ", lefttoplace);
						rtoplace = iNumLeftToPlace + lefttoplace;
						RndTable[region_number] = rtoplace;
	...
	for region_number, res_ID in ipairs(self.region_luxury_assignment) do
		...
		local iNumThisLuxToPlace = math.max(2, targetNum); -- Always place at least one.

		--print("-"); print("Target number for Luxury#", res_ID, "with assignment split of", assignment_split, "is", targetNum);
		
		-- Place luxuries.
		shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[primary])
		iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumThisLuxToPlace, 0.3, 2, 3, 4, shuf_list);
		if iNumLeftToPlace > 0 and secondary > 0 then
			shuf_list = GetShuffledCopyOfTable(luxury_plot_lists[secondary])
	...
	if self.iNumTypesRandom > 0 then
		...
		for loop, res_ID in ipairs(self.resourceIDs_assigned_to_random) do
			...
			if self.iNumTypesRandom * 3 > iNumRandomLuxTarget then
				iNumThisLuxToPlace = 2 + Map.Rand(3, "");
			...
			iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumThisLuxToPlace, 0.5, 2, 4, 0, current_list);
			if iNumLeftToPlace > 0 and secondary > 0 then
				current_list = self.global_luxury_plot_lists[secondary];
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumLeftToPlace, 0.5, 2, 3, 0, current_list);
			end
			if iNumLeftToPlace > 0 and tertiary > 0 then
			...
			if iNumLeftToPlace > 0 and quaternary > 0 then
				...
				iNumLeftToPlace = self:PlaceSpecificNumberOfResources(res_ID, 1, iNumLeftToPlace, 0.75, 2, 3, 0, current_list);
			end
			...
			for index_to_check = current_index, iNumTotalPlots do
				...
				if self.fishData[plotIndex] == 0 then
					...
					if res_plot:GetResourceType(-1) == -1 then
						...
						local fish_radius = Map.Rand(4, "Fish Radius - Place Fish LUA");
...
function AssignStartingPlots:PlaceSexyBonusAtCivStarts()
	...
	--Random Sexy Bonus
	-- 0. Undefined
	bonus_type_associated_with_region_type[0] = self.bison_ID;
	
	-- 1. Tundra
	if Map.Rand(2,"") == 0 then
		bonus_type_associated_with_region_type[1] = self.stone_ID;
	else
		bonus_type_associated_with_region_type[1] = self.deer_ID;
	end
	
	-- 2. Jungle
	if Map.Rand(1, "") == 0 then
		bonus_type_associated_with_region_type[2] = self.hardwood_ID;
	else
		bonus_type_associated_with_region_type[2] = self.banana_ID;
	end
	
	-- 3. Forest
	if Map.Rand(2, "") == 0 then
		bonus_type_associated_with_region_type[3] = self.hardwood_ID;
	else
		bonus_type_associated_with_region_type[3] = self.deer_ID;
	end
	
	-- 4. Desert
	if Map.Rand(3, "") == 0 then
		bonus_type_associated_with_region_type[4] = self.stone_ID;
	else
		bonus_type_associated_with_region_type[4] = self.wheat_ID;
	end
	
	-- 5. Hills
	bonus_type_associated_with_region_type[5] = self.sheep_ID;
	
	-- 6. Plains
	if Map.Rand(2, "") == 0 then
		bonus_type_associated_with_region_type[6] = self.bison_ID;
	else
		bonus_type_associated_with_region_type[6] = self.wheat_ID;
	end	
	
	-- 7. Grassland
	local x = Map.Rand(2,"");
	if x == 1 then
		bonus_type_associated_with_region_type[7] = self.cow_ID;
	elseif x == 2 then
		bonus_type_associated_with_region_type[7] = self.stone_ID;
	else
		bonus_type_associated_with_region_type[7] = self.maize_ID;
	end	
	
	-- 8. Hybrid
	bonus_type_associated_with_region_type[8] = self.bison_ID;
	
	-- 9. Marsh
	bonus_type_associated_with_region_type[9] = self.bison_ID;
			...
			for plot_to_handle = 1, ripple_radius do
				...
				elseif wrapY == false and (nextY < 0 or nextY >= iH) then
					...
					if plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS then -- No resource or Oasis here, safe to proceed.
						...
						elseif use_this_ID == self.bison_ID then
							if terrainType == TerrainTypes.TERRAIN_PLAINS and terrainType == TerrainTypes.TERRAIN_GRASS and plotType == PlotTypes.PLOT_LAND then
								if featureType == FeatureTypes.NO_FEATURE then
									table.insert(plot_list, plotIndex);
								end
							end
						...
						elseif use_this_ID == self.stone_ID then
							if terrainType == TerrainTypes.TERRAIN_GRASS and plotType == PlotTypes.PLOT_LAND then
								if featureType == FeatureTypes.NO_FEATURE then
									table.insert(plot_list, plotIndex);
								end
						...
						elseif use_this_ID == self.maize_ID then
							if terrainType == TerrainTypes.TERRAIN_GRASS and plotType == PlotTypes.PLOT_LAND and featureType == FeatureTypes.NO_FEATURE then
								table.insert(plot_list, plotIndex);
							end
						end
						if plotType == PlotTypes.PLOT_OCEAN then
...
------------------------------------------------------------------------------
function AssignStartingPlots:AddForestToResource()
	print("AddForestToResource");
	local iW, iH = Map.GetGridSize()
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			local plot = Map.GetPlot(x, y)
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print("- Hardwood: ", self.amounts_of_resources_placed[self.hardwood_ID + 1]);
	print("- Maize...: ", self.amounts_of_resources_placed[self.maize_ID + 1]);
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	{self.uranium_ID, uran_amt, 35, 10, 0} };
	...
	{self.uranium_ID, uran_amt, 70, 10, 0} };
	...
	while self.amounts_of_resources_placed[self.uranium_ID + 1] < 4 * self.iNumCivs do
		print("Map has very low uranium, adding another.");
		local resources_to_place = { {self.uranium_ID, uran_amt, 100, 15, 0} };
		self:ProcessResourceList(99999, 1, self.land_list, resources_to_place)
	end
	...
	self:PlaceFish(5 * bonus_multiplier, self.coast_list);
	self:PlaceFish(3 * bonus_multiplier, self.front_coast_list);
	...
	self:ProcessResourceList(16 * bonus_multiplier, 3, self.dry_grass_flat_no_feature, resources_to_place)

	local resources_to_place = {
	{self.sheep_ID, 1, 100, 1, 1} };
	self:ProcessResourceList(8 * bonus_multiplier, 3, self.hills_open_list, resources_to_place)
	...
	self:ProcessResourceList(10 * bonus_multiplier, 3, self.forest_flat_that_are_not_tundra, resources_to_place)
	
	local resources_to_place = {
	{self.hardwood_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.hills_covered_list, resources_to_place)

	local resources_to_place = {
	{self.hardwood_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.forest_flat_list, resources_to_place)

	local resources_to_place = {
	{self.hardwood_ID, 1, 100, 1, 2} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.jungle_flat_list, resources_to_place)
...
function AssignStartingPlots:PlaceResourcesAndCityStates()
	...
	-- add forest to certain resources
	self:AddForestToResource()
